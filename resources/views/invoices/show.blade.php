<x-app-layout>
    <x-slot name="header">
        <x-crud.page-header eyebrow="Finance Workspace" :title="$invoice->invoice_number" description="Invoice detail with customer, payment exposure, and line-level billing context." :back-url="route('finance.invoices.index')" back-label="Back to Invoices">
            <a href="{{ route('finance.invoices.edit', $invoice) }}" class="btn btn-primary">Edit Invoice</a>
        </x-crud.page-header>
    </x-slot>
        ...
        <div class="grid gap-6 xl:grid-cols-[minmax(0,1.4fr)_minmax(320px,0.9fr)]">
            ...
            <section class="app-card app-card-body">
                ...
                <div class="mt-5 grid gap-4">
                    <div class="app-surface px-4 py-4"><p class="text-xs uppercase tracking-[0.16em] text-slate-500">Subtotal</p><p class="mt-2 text-lg font-bold text-slate-950">₹{{ number_format((float) $invoice->subtotal, 2) }}</p></div>
                    <div class="app-surface px-4 py-4"><p class="text-xs uppercase tracking-[0.16em] text-slate-500">Tax</p><p class="mt-2 text-lg font-bold text-slate-950">₹{{ number_format((float) $invoice->tax_amount, 2) }}</p></div>
                    <div class="app-surface px-4 py-4"><p class="text-xs uppercase tracking-[0.16em] text-slate-500">Total</p><p class="mt-2 text-lg font-bold text-slate-950">₹{{ number_format((float) $invoice->total, 2) }}</p></div>
                    <div class="app-surface px-4 py-4"><p class="text-xs uppercase tracking-[0.16em] text-slate-500">Balance Due</p><p class="mt-2 text-lg font-bold text-slate-950">₹{{ number_format((float) $invoice->balance_due, 2) }}</p></div>
                </div>
            </section>
        </div>
