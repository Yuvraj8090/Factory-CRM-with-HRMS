<x-app-layout>
    ...
    <div class="grid gap-6 md:grid-cols-3">
        <section class="app-card app-card-body"><p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Provider ID</p><p class="mt-2 text-base font-semibold text-slate-950">{{ $whatsAppTemplate->template_id }}</p></section>
        <section class="app-card app-card-body"><p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Category</p><p class="mt-2 text-base font-semibold text-slate-950">{{ $whatsAppTemplate->category ?: 'Uncategorized' }}</p></section>
        <section class="app-card app-card-body"><p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Status</p><div class="mt-2"><x-crud.status-badge :value="$whatsAppTemplate->is_active ? 'Active' : 'Inactive'" /></div></section>
        <section class="app-card app-card-body md:col-span-3">
