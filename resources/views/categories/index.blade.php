<x-app-layout>
    ...
    <section class="app-card">
        <div class="overflow-x-auto">
            <table id="categories-table" class="table table-hover app-data-table text-sm" data-datatable-url="{{ route('settings.categories.index') }}" data-datatable-storage-key="categories" data-datatable-columns='{{ \Illuminate\Support\Js::from([["data"=>"name","name"=>"name"],["data"=>"parent_name","name"=>"parent.name","searchable"=>false],["data"=>"items_total","name"=>"items_count"],["data"=>"status_badge","name"=>"is_active","orderable"=>false,"searchable"=>false],["data"=>"actions","name"=>"id","orderable"=>false,"searchable"=>false]]) }}'>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($categories as $category)
                        <tr class="transition hover:bg-slate-50/80">
                            ...
                            <td class="px-6 py-5"><div class="flex justify-end gap-2"><a href="{{ route('settings.categories.show', $category) }}" class="btn btn-outline-secondary btn-sm">View</a><a href="{{ route('settings.categories.edit', $category) }}" class="btn btn-outline-secondary btn-sm">Edit</a></div></td>
                        </tr>
                    @empty
